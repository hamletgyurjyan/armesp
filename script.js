...
function translateFormElements(lang) {
  ...
  if (arrDisplay && !arrDisplay.classList.contains('has-value')) arrDisplay.textContent = t['form_select_time'] || 'Select Time';
  const retDisplay = document.getElementById('tp-return-display');
  if (retDisplay && !retDisplay.classList.contains('has-value')) retDisplay.textContent = t['form_select_time'] || 'Select Time';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (blogGrid && loadMoreBtn) {
    const articles = Array.from(blogGrid.querySelectorAll('.article-link'));
    ...
    loadMoreBtn.addEventListener('click', function (e) {
      ...
      if (articles[firstNewIndex]) {
        const yCoordinate = articles[firstNewIndex].getBoundingClientRect().top + window.pageYOffset - 100;
  ...
  function initHomePage() {
    ...
    function scrollToForm() {
      ...
      if (target) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            target.classList.remove('form-highlight');
          }, 2000);
        }, 600);
      }
    }
    ...
    // Handle dynamically rendered hotel card buttons
    document.addEventListener('click', (e) => {
...
function renderCal(id) {
  state[id].month = parseInt(document.getElementById('month-' + id).value);
  state[id].year = parseInt(document.getElementById('year-' + id).value);
...
function selectDay(id, d) {
  ...
  disp.textContent = val; disp.classList.remove('placeholder');
  document.getElementById(id + '-date').value = `${s.year}-${String(s.month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
  renderCal(id);
...
initCal('arrival'); initCal('return');
...
function change(key, delta) {
  ...
  document.getElementById(key + '-val').textContent = statePerson[key];
...
function travelDateField() {
  return firstFieldEl('pickup-date', 'arrival-date');
}
...
function wrapEl(id) {
  const normalized = id.includes('location') || id.includes('date') || id.includes('time')
    ? id
    : `${id}-location`;

  const input = document.getElementById(normalized);
...
function validateReturnDateAgainstPickup(returnDateId, returnWrapId, pickupDateId) {
  const returnInput = fieldEl(returnDateId);
  const returnWrap = document.getElementById('field-' + returnWrapId);
  ...
  if (returnDate < pickupDate) {
    showError(returnWrap, 'Return date cannot be before arrival date');
    return false;
  }

  clearError(returnWrap);
  return true;
}
...
document.getElementById('add-return-toggle').addEventListener('click', () => {
  isRoundTrip = true;
  document.getElementById('return-section').classList.add('visible');
  document.getElementById('add-return-toggle').style.display = 'none';
});

document.getElementById('delete-return-btn').addEventListener('click', () => {
  isRoundTrip = false;
  document.getElementById('return-section').classList.remove('visible');
  document.getElementById('add-return-toggle').style.display = 'flex';
  fieldEl('return-date').value = '';
  fieldEl('return-time').value = '';
  clearError(document.getElementById('field-return-date'));
  clearError(document.getElementById('field-return-time'));
});
...
document.getElementById('go-step-2').addEventListener('click', () => {
  ...
  const timeVal = fieldEl('pickup-time').value;
  if (!timeVal) { showError(document.getElementById('pickup-time'), translations[currentLang]['field_required']); valid = false; }
  else clearError(document.getElementById('pickup-time'));

  if (isRoundTrip) {
    valid = validateDate('return-date') && valid;
    if (mainDateInput) {
      valid = validateReturnDateAgainstPickup('return-date', 'return-date', mainDateInput.id) && valid;
    }
    const retTime = fieldEl('return-time').value;
    if (!retTime) { showError(document.getElementById('return-time'), translations[currentLang]['field_required']); valid = false; }
    else clearError(document.getElementById('return-time'));
  }
...
document.getElementById('go-step-1').addEventListener('click', () => {
...
document.getElementById('resetBtn').addEventListener('click', () => {
  ['pickup-location', 'dropoff-location', 'pickup-date', 'arrival-date',
   'pickup-time', 'return-time', 'flight-number', 'return-date',
   'full-name', 'phone-number', 'email-address', 'extra-requests'
  ].forEach(id => {
    const el = fieldEl(id);
    if (el) el.value = '';
  });
  ...
  document.getElementById('adults-minus').disabled   = true;
  document.getElementById('children-minus').disabled = true;
  document.getElementById('lug10-minus').disabled    = true;
  document.getElementById('lug20-minus').disabled    = true;
  ...
  ['arrival','return'].forEach(id => {
    ...
    if (disp) {
      const t = translations[currentLang] || translations['en'];
      disp.textContent = t['form_select_time'] || 'Select Time';
      disp.style.color = '';
    }
...
function selectPhoneCountry(country) {
  document.getElementById('phone-code').textContent = country.code;
  ...
  document.getElementById('phone-number').placeholder =
    country.placeholder.replace(/#/g, 'X');
...
function filterPhoneList(q) {
  ...
  document.querySelectorAll('.phone-dropdown-item').forEach(function (li) {
    const match = li.dataset.name.includes(term) || li.dataset.code.includes(term);
...
(function () {
  ...
  function buildTimePicker(id) {
    ...
    var hc = document.getElementById('tp-' + id + '-hours');
    var mc = document.getElementById('tp-' + id + '-mins');
    if (!hc || !mc) return;
  ...
  function pickTime(id, type, val, el) {
    var col = document.getElementById('tp-' + id + '-' + (type === 'h' ? 'hours' : 'mins'));
    col.querySelectorAll('.time-item').forEach(function (x) { x.classList.remove('selected'); });
    ...
    if (h !== null && m !== null) {
      var txt = pad(h) + ':' + pad(m);
      document.getElementById('tp-' + id + '-display').textContent = txt;
      document.getElementById('tp-' + id + '-display').style.color = '';
      document.getElementById('tp-' + id).classList.add('has-value');

      var hidden = id === 'arrival' ? document.getElementById('pickup-time')
  ...
  window.toggleTimePicker = function (id) {
    var panel = document.getElementById('tp-' + id + '-panel');
    var trigger = document.getElementById('tp-' + id);
  ...
  function closeAllTimePickers() {
    document.querySelectorAll('.time-panel').forEach(function (p) { p.classList.remove('open'); });
    document.querySelectorAll('.time-trigger').forEach(function (t) { t.classList.remove('open'); });
  }

  document.addEventListener('click', function (e) {
    if (!e.target.closest('[id^="wrap-arrival"]') &&
      !e.target.closest('[id^="wrap-return"]')) {
      closeAllTimePickers();
    }
  });

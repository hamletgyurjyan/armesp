function handleSectionJump() {
    ...
    // If there is no ?section= in the URL, stop here
    if (!targetId) return;

    const targetElement = document.getElementById(targetId);
    if (targetElement) {
        // We use a 400ms delay to give the browser time to finish 
        // adding the hotel cards to the page.
        setTimeout(() => {
...
function setLanguage(lang) {
    ...
    // Update UI buttons color if they exist (Simple EN/ES buttons)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Only run this if we are on a page that actually has a blog grid
    if (blogGrid && loadMoreBtn) {
        const articles = Array.from(blogGrid.querySelectorAll('.article-link'));
        
        console.log('Blog system initialized. Total articles found:', articles.length);
        ...
        // Run immediately to hide items 10+
        updateVisibility();
    }
